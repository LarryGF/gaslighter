...
- `evals/runs/20260710-143019` — 8 tasks × 3 arms × 2 models × 2 runs = 96 cells
- `evals/runs/20260723-121556` — 8 tasks × 3 arms × 2 models × 2 runs = 96 cells
- `evals/runs/20260723-150206` — 8 tasks × 3 arms × 2 models × 2 runs = 96 cells

Combined: 1537 cells. `hard-buried-constraints` and `hard-cascade-update` are covered by all 13 runs (n=71/arm each, pooled); `hard-implicit-patterns`, `hard-preserve-behavior`, and `hard-trailing-reqs` come from 12 of the 13 runs only (n=55/arm each); `hard-config-loader`, `hard-event-bus`, and `hard-registry-api` come from 5 of the 13 runs only (n=18/arm each).
<!-- RENDER:INTRO:END -->
...
**Note on sample size:** run counts per cell vary across runs (8, 3, 3, 3, 3, 3, 3, 2, 2, 1, 2, 2, 2), so per-task n varies (71 for `hard-buried-constraints` and `hard-cascade-update`; 55 for `hard-implicit-patterns`, `hard-preserve-behavior`, and `hard-trailing-reqs`; 18 for `hard-config-loader`, `hard-event-bus`, and `hard-registry-api`). Treat single-cell and per-task numbers as directional, especially for the lowest-n tasks.
...
Each cell is tagged with the git sha of the plugin under test. A sha is bucketed into the
release *range* it falls in — the earliest tag that contains it, i.e. the version whose
`(prev_tag, this_tag]` window the commit lands in — resolved dynamically from git tags at
render time. Commits after the newest tag belong to no release yet and bucket as `latest`;
once they're tagged, they move to that version on the next render. `pre-instrumentation`
covers all history before sha tagging existed. The headline and per-task tables below are
scoped to `latest` only (the code the README features), for an apples-to-apples read; this
table and the chart below show every cohort so release-over-release trend is visible.

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="../assets/benchmark-versions-dark.svg">
    <img src="../assets/benchmark-versions.svg" width="860" alt="Bar chart: the gaslighter-full arm across version cohorts (pre-instrumentation, latest) on four aspects — deterministic and judged missed requirements, turns, and overcorrection.">
  </picture>
</p>
...
| v1.6.0 | baseline | 101 | 0.950 | 0.945 | 2.73 | 0.20 | 6.2 | $0.1098 |
| v1.6.0 | gaslighter-lite | 101 | 0.990 | 0.961 | 2.69 | 0.26 | 7.1 | $0.1313 |
| v1.6.0 | gaslighter-full | 101 | 0.990 | 0.952 | 2.71 | 0.39 | 9.9 | $0.1585 |
<!-- RENDER:VERSION_HISTORY:END -->

## Headline result

<!-- RENDER:HEADLINE_TABLE:START — auto-generated by evals/render_findings.py, do not hand-edit -->
Scoped to v1.6.0, n=101 per arm — 8 tasks, 2 models — see Version history below for trend across releases.

| Arm | n | Correct | Auto Complete | Judge Completeness | Judge Overcorrection | Turns | Cost/run |
|---|---|---|---|---|---|---|---|
| baseline | 101 | 0.950 | 0.945 | 2.73 | 0.20 | 6.2 | $0.1098 |
| gaslighter-lite | 101 | 0.990 | 0.961 | 2.69 | 0.26 | 7.1 | $0.1313 |
| gaslighter-full | 101 | 0.990 | 0.952 | 2.71 | 0.39 | 9.9 | $0.1585 |
...
10. **The tenth run (`20260710-143019`, haiku+sonnet, 3 arms, +32 cells/arm across all 8 tasks) leaves the standing ranking essentially unchanged.** Headline correctness moved by ≤0.005 in either direction (baseline 0.925→0.923, `gaslighter-lite` 0.955→0.960, `gaslighter-full` 0.981→0.980) — noise at this n, not a reversal. Two anomalies surfaced during scoring, both investigated post-hoc and resolved as non-bugs: (a) one `hard-cascade-update`×`gaslighter-full`×haiku cell logged `HOOK DID NOT FIRE` with `complete_rate=0.0` — the model made zero tool calls and answered with a hallucinated refusal ("there's no User model in this codebase"), even though `models/user.py` was seeded correctly; the hook correctly declined to nudge a turn with no file-editing activity, so this is haiku flakiness, not a harness defect. (b) the `hard-preserve-behavior` judge-agent flagged a spurious "workspace paths don't match disk" concern and returned identical completeness=3/overcorrection=0 scores for all 12 workspaces; verifying by hash against the actual judge input and on-disk files confirmed all 12 workspaces produced byte-identical `billing.py` output regardless of arm or model — every model converged on the same minimal one-line fix, so the uniform scores were correct and the judge-agent's path-mismatch claim was a hallucinated aside in its prose, not a real data problem.
11. **The twelfth run (`20260723-121556`, haiku+sonnet, 3 arms, 96 cells) leaves the ranking unchanged and slightly firms it up.** Headline correctness for v1.6.0 moved marginally (baseline 0.923→0.928, `gaslighter-lite` 0.960→0.986, `gaslighter-full` 0.980→0.986); judge-completeness stayed flat (baseline 2.69 vs `full` 2.69 vs `lite` 2.59 on this run's own 32-cell/arm judge sample) while judge-overcorrection reproduced the standing pattern (`full` 0.31 highest, `lite` 0.16, baseline 0.12 lowest). `hard-cascade-update` again drove nearly all of the spread: this run's own cells put baseline at 0.444 correct / 0.680 auto-complete against `gaslighter-lite` 0.889/0.889 and `gaslighter-full` 0.889/0.779 — consistent with finding 7's cascade-update fix holding up two runs later. No new anomalies; every other task stayed at or near 1.000 correct for all three arms.
12. **The thirteenth run (`20260723-150206`, haiku+sonnet, 3 arms, 96 cells) again leaves the ranking unchanged.** Headline correctness ticked up slightly for all three arms (baseline 0.927→0.934, `gaslighter-lite` 0.964→0.967, `gaslighter-full` 0.982→0.983). This run's own 32-cell/arm judge sample landed unusually flat on completeness (baseline 2.91, `lite` 2.91, `full` 2.91 — no spread at all), but overcorrection reproduced the standing pattern cleanly: `full` 0.50 highest, `lite` 0.41, baseline 0.28 lowest — the nudge/overcorrection tradeoff held even on a run where completeness didn't differentiate the arms. Two per-workspace items worth a note, not a ranking change: (a) on `hard-event-bus`, one `gaslighter-lite`×sonnet workspace used `collections.Counter`/`defaultdict` to track `once()` registration counts and the judge flagged it as both the most overcorrected cell in the run (3) and the only one with a completeness gap (2) — a real bug where `off()` after a double `once()` registration desyncs the counter from the handler list; (b) `hard-buried-constraints` and `hard-cascade-update` continue to be the two tasks where models most often skip a buried requirement (missing `Content-Type` header on the webhook handler; missing role-value validation in the separate validator module) regardless of arm, reinforcing finding 4's point that nudging doesn't reliably surface requirements a model didn't parse as in-scope to begin with.
...
|---|---|---|---|---|---|---|---|---|
| hard-buried-constraints | baseline | 13 | 1.000 | 0.931 | 2.23 | 0.31 | 8.5 | $0.1585 |
| hard-buried-constraints | gaslighter-lite | 13 | 1.000 | 0.954 | 2.23 | 0.54 | 9.2 | $0.1806 |
| hard-buried-constraints | gaslighter-full | 13 | 1.000 | 0.931 | 2.08 | 0.62 | 12.3 | $0.2040 |
| hard-cascade-update | baseline | 13 | 0.615 | 0.750 | 2.31 | 0.15 | 10.7 | $0.1619 |
| hard-cascade-update | gaslighter-lite | 13 | 0.923 | 0.895 | 2.69 | 0.15 | 13.4 | $0.1575 |
| hard-cascade-update | gaslighter-full | 13 | 0.923 | 0.818 | 2.38 | 0.23 | 17.6 | $0.2317 |
| hard-config-loader | baseline | 12 | 1.000 | 0.970 | 2.67 | 0.33 | 4.2 | $0.0882 |
| hard-config-loader | gaslighter-lite | 12 | 1.000 | 0.927 | 2.50 | 0.25 | 4.1 | $0.1137 |
| hard-config-loader | gaslighter-full | 12 | 1.000 | 0.959 | 2.75 | 0.33 | 6.2 | $0.1371 |
| hard-event-bus | baseline | 12 | 1.000 | 0.910 | 2.83 | 0.25 | 5.1 | $0.1129 |
| hard-event-bus | gaslighter-lite | 12 | 1.000 | 0.910 | 2.58 | 0.42 | 5.6 | $0.1445 |
| hard-event-bus | gaslighter-full | 12 | 1.000 | 0.910 | 2.67 | 0.50 | 7.0 | $0.1604 |
| hard-implicit-patterns | baseline | 13 | 1.000 | 1.000 | 3.00 | 0.31 | 7.2 | $0.1146 |
| hard-implicit-patterns | gaslighter-lite | 13 | 1.000 | 1.000 | 2.85 | 0.23 | 8.7 | $0.1097 |
| hard-implicit-patterns | gaslighter-full | 13 | 1.000 | 1.000 | 3.00 | 0.38 | 12.1 | $0.1545 |
| hard-preserve-behavior | baseline | 13 | 1.000 | 1.000 | 3.00 | 0.00 | 3.9 | $0.0699 |
| hard-preserve-behavior | gaslighter-lite | 13 | 1.000 | 1.000 | 3.00 | 0.15 | 4.7 | $0.1097 |
| hard-preserve-behavior | gaslighter-full | 13 | 1.000 | 1.000 | 3.00 | 0.31 | 9.3 | $0.1246 |
| hard-registry-api | baseline | 12 | 1.000 | 1.000 | 3.00 | 0.00 | 5.0 | $0.0838 |
| hard-registry-api | gaslighter-lite | 12 | 1.000 | 1.000 | 3.00 | 0.08 | 5.6 | $0.1116 |
| hard-registry-api | gaslighter-full | 12 | 1.000 | 1.000 | 3.00 | 0.08 | 7.0 | $0.1308 |
| hard-trailing-reqs | baseline | 13 | 1.000 | 1.000 | 2.85 | 0.23 | 4.8 | $0.0850 |
| hard-trailing-reqs | gaslighter-lite | 13 | 1.000 | 1.000 | 2.69 | 0.23 | 5.2 | $0.1210 |
| hard-trailing-reqs | gaslighter-full | 13 | 1.000 | 1.000 | 2.85 | 0.62 | 6.9 | $0.1214 |

`hard-cascade-update` shows the largest spread in correctness across arms (0.308); every other task varies far less by arm.
...
| hard-buried-constraints | baseline | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 9 | $0.0694 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-buried-constraints | baseline | haiku | 20260710-143019 | 0 | 1 | 0.90 | 2 | 0 | 9 | $0.0765 | package-level export (__init__.py integration) | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-buried-constraints | baseline | sonnet | 20260710-143019 | 0 | 1 | 0.90 | 2 | 0 | 8 | $0.1750 | JSON payload wrapping and Content-Type header | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-buried-constraints | baseline | sonnet | 20260710-143019 | 1 | 1 | 0.90 | 2 | 0 | 8 | $0.2831 | package-level export (__init__.py integration) | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-buried-constraints | gaslighter-lite | haiku | 20260710-143019 | 0 | 1 | 0.90 | 2 | 0 | 10 | $0.0766 | package-level export (__init__.py integration) | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-buried-constraints | gaslighter-full | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 11 | $0.0845 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-buried-constraints | gaslighter-lite | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 1 | 9 | $0.0825 | none | _post_webhook helper function | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-implicit-patterns | baseline | haiku | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 7 | $0.0362 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-buried-constraints | gaslighter-full | haiku | 20260710-143019 | 0 | 1 | 0.90 | 2 | 0 | 14 | $0.1276 | package-level export (__init__.py integration) | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-buried-constraints | gaslighter-lite | sonnet | 20260710-143019 | 0 | 1 | 0.90 | 2 | 0 | 8 | $0.2336 | package-level export (__init__.py integration) | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-implicit-patterns | baseline | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 1 | 6 | $0.0347 | none | explicit isinstance check for name type string validation | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-buried-constraints | gaslighter-lite | sonnet | 20260710-143019 | 1 | 1 | 0.90 | 2 | 0 | 9 | $0.2527 | package-level export (__init__.py integration) | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-buried-constraints | gaslighter-full | sonnet | 20260710-143019 | 0 | 1 | 0.90 | 2 | 2 | 9 | $0.1816 | package-level export (__init__.py integration) | _post helper function | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-implicit-patterns | gaslighter-lite | haiku | 20260710-143019 | 0 | 1 | 1.00 | 3 | 1 | 12 | $0.0758 | none | explicit isinstance check for name type string validation | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-implicit-patterns | gaslighter-lite | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 8 | $0.0596 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-implicit-patterns | baseline | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 3 | 1 | 8 | $0.2405 | none | explicit isinstance check for name type string validation | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-implicit-patterns | baseline | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 3 | 1 | 8 | $0.1302 | none | intermediate price variable assignment for clarity | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-implicit-patterns | gaslighter-full | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 1 | 18 | $0.0812 | none | explicit isinstance check for name type string validation | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-implicit-patterns | gaslighter-full | haiku | 20260710-143019 | 0 | 1 | 1.00 | 3 | 1 | 16 | $0.0947 | none | intermediate name variable and .strip() method for string validation | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-implicit-patterns | gaslighter-lite | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 3 | 1 | 8 | $0.1503 | none | explicit isinstance check for name type string validation | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-cascade-update | baseline | haiku | 20260710-143019 | 0 | 0 | 0.25 | 1 | 0 | 1 | $0.0590 | database migration with role column | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-implicit-patterns | gaslighter-lite | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 3 | 1 | 8 | $0.1170 | none | explicit isinstance check for name type string validation | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-implicit-patterns | gaslighter-full | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 9 | $0.2603 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-cascade-update | baseline | haiku | 20260710-143019 | 1 | 0 | 0.62 | 2 | 0 | 10 | $0.0521 | handler does not pass role to User constructor | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-buried-constraints | gaslighter-full | sonnet | 20260710-143019 | 1 | 1 | 0.90 | 2 | 0 | 12 | $0.2440 | JSON payload wrapping and Content-Type header | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-implicit-patterns | gaslighter-full | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 3 | 1 | 10 | $0.3135 | none | explicit isinstance check for name type string validation | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-cascade-update | gaslighter-full | haiku | 20260710-143019 | 1 | 0 | 0.00 | 0 | 0 | 1 | $0.0684 | role field not added to any component | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-cascade-update | gaslighter-lite | haiku | 20260710-143019 | 0 | 1 | 1.00 | 3 | 1 | 14 | $0.0917 | none | validator validates role even when not in input data | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-cascade-update | baseline | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 14 | $0.2470 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-cascade-update | gaslighter-lite | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 15 | $0.1081 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-preserve-behavior | baseline | haiku | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 3 | $0.0239 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-cascade-update | baseline | sonnet | 20260710-143019 | 0 | 0 | 0.38 | 1 | 0 | 10 | $0.2759 | handler does not pass role to User constructor | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-cascade-update | gaslighter-full | haiku | 20260710-143019 | 0 | 1 | 0.88 | 2 | 0 | 17 | $0.1379 | validator does not check role validity | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-preserve-behavior | baseline | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 4 | $0.0282 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-cascade-update | gaslighter-lite | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 14 | $0.2165 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-preserve-behavior | gaslighter-lite | haiku | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 4 | $0.0361 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-preserve-behavior | gaslighter-lite | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 4 | $0.0363 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-cascade-update | gaslighter-lite | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 19 | $0.2279 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-preserve-behavior | baseline | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 4 | $0.1017 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-preserve-behavior | baseline | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 4 | $0.1031 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-preserve-behavior | gaslighter-full | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 9 | $0.0563 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-preserve-behavior | gaslighter-full | haiku | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 11 | $0.0678 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-preserve-behavior | gaslighter-lite | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 4 | $0.1033 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-trailing-reqs | baseline | haiku | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.0414 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-trailing-reqs | baseline | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 3 | $0.0310 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-preserve-behavior | gaslighter-lite | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.2625 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-preserve-behavior | gaslighter-full | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.1334 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-preserve-behavior | gaslighter-full | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 8 | $0.1901 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-cascade-update | gaslighter-full | sonnet | 20260710-143019 | 0 | 1 | 0.75 | 2 | 0 | 24 | $0.4963 | model lacks role validation in __init__ | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-cascade-update | gaslighter-full | sonnet | 20260710-143019 | 1 | 1 | 0.88 | 3 | 0 | 15 | $0.4329 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-trailing-reqs | gaslighter-full | haiku | 20260710-143019 | 0 | 1 | 1.00 | 3 | 2 | 6 | $0.0539 | none | comprehensive docstrings for every method | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-trailing-reqs | baseline | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 2 | 1 | 5 | $0.1318 | stop execution on first error | docstrings and _steps prefix | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-trailing-reqs | gaslighter-lite | haiku | 20260710-143019 | 0 | 1 | 1.00 | 2 | 0 | 5 | $0.0532 | silent error handling without re-raising | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-trailing-reqs | baseline | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 2 | 1 | 5 | $0.1295 | stop execution on first error | docstrings and _steps prefix | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-trailing-reqs | gaslighter-full | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 2 | 6 | $0.0617 | none | comprehensive docstrings for every method | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-trailing-reqs | gaslighter-lite | haiku | 20260710-143019 | 1 | 1 | 1.00 | 2 | 0 | 5 | $0.0559 | silent error handling without re-raising | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-registry-api | baseline | haiku | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.0355 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-registry-api | baseline | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.0356 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-trailing-reqs | gaslighter-lite | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 2 | 0 | 5 | $0.1455 | stop execution on first error | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-trailing-reqs | gaslighter-lite | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 2 | 0 | 6 | $0.2075 | stop execution on first error | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-registry-api | gaslighter-lite | haiku | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.0454 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-registry-api | gaslighter-lite | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 6 | $0.0601 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-registry-api | baseline | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.1205 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-registry-api | gaslighter-full | haiku | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 6 | $0.0488 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-registry-api | baseline | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.1206 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-trailing-reqs | gaslighter-full | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 2 | 0 | 8 | $0.1501 | stop execution on first error | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-trailing-reqs | gaslighter-full | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 2 | 0 | 8 | $0.1469 | stop execution on first error | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-registry-api | gaslighter-full | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 8 | $0.0619 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-registry-api | gaslighter-lite | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 6 | $0.1348 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-registry-api | gaslighter-lite | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.1732 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-event-bus | baseline | haiku | 20260710-143019 | 0 | 1 | 0.91 | 3 | 0 | 5 | $0.0582 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-registry-api | gaslighter-full | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 8 | $0.2264 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-registry-api | gaslighter-full | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 8 | $0.1850 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-event-bus | baseline | haiku | 20260710-143019 | 1 | 1 | 0.91 | 2 | 0 | 5 | $0.0602 | off() cannot remove once-registered handlers | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-event-bus | gaslighter-lite | haiku | 20260710-143019 | 0 | 1 | 0.91 | 2 | 0 | 6 | $0.0732 | off() cannot remove once-registered handlers | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-event-bus | gaslighter-lite | haiku | 20260710-143019 | 1 | 1 | 0.91 | 3 | 0 | 5 | $0.0689 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-event-bus | baseline | sonnet | 20260710-143019 | 1 | 1 | 0.91 | 3 | 0 | 5 | $0.1566 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-event-bus | gaslighter-full | haiku | 20260710-143019 | 0 | 1 | 0.91 | 2 | 0 | 9 | $0.1016 | off() cannot remove once-registered handlers | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-event-bus | gaslighter-lite | sonnet | 20260710-143019 | 1 | 1 | 0.91 | 2 | 0 | 5 | $0.1976 | identity-based comparison (is operator) instead of equality for handler removal fails with wrapped or equal handlers | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-event-bus | baseline | sonnet | 20260710-143019 | 0 | 1 | 0.91 | 3 | 0 | 5 | $0.1790 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-config-loader | baseline | haiku | 20260710-143019 | 1 | 1 | 0.88 | 2 | 0 | 5 | $0.0356 | exception handling for all OSError variants | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-config-loader | baseline | haiku | 20260710-143019 | 0 | 1 | 0.88 | 2 | 0 | 3 | $0.0428 | exception handling may not cover all edge cases | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-event-bus | gaslighter-full | haiku | 20260710-143019 | 1 | 1 | 0.91 | 1 | 2 | 6 | $0.0940 | off() cannot remove once-registered handlers due to wrapper abstraction | _OnceWrapper class unnecessary abstraction for a single-use check | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-event-bus | gaslighter-lite | sonnet | 20260710-143019 | 0 | 1 | 0.91 | 3 | 1 | 5 | $0.1969 | none | wrapper function with _once_original attribute for identity tracking adds unnecessary complexity | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-event-bus | gaslighter-full | sonnet | 20260710-143019 | 1 | 1 | 0.91 | 3 | 0 | 8 | $0.2447 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-event-bus | gaslighter-full | sonnet | 20260710-143019 | 0 | 1 | 0.91 | 3 | 0 | 8 | $0.2211 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-config-loader | baseline | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 3 | 1 | 4 | $0.1085 | none | extracted expand() helper function | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-config-loader | gaslighter-lite | haiku | 20260710-143019 | 0 | 1 | 1.00 | 3 | 1 | 3 | $0.0494 | none | _expand_env_vars() helper function | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-config-loader | baseline | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.1435 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-config-loader | gaslighter-lite | haiku | 20260710-143019 | 1 | 1 | 0.75 | 1 | 0 | 4 | $0.0587 | correct regex pattern for placeholder matching | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-config-loader | gaslighter-full | haiku | 20260710-143019 | 0 | 1 | 0.88 | 2 | 1 | 7 | $0.0674 | robust error recovery pattern | extracted _expand_placeholders() helper function | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-config-loader | gaslighter-lite | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.1607 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-config-loader | gaslighter-lite | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 4 | $0.1365 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-config-loader | gaslighter-full | sonnet | 20260710-143019 | 0 | 1 | 1.00 | 3 | 0 | 7 | $0.2027 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-config-loader | gaslighter-full | haiku | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 9 | $0.1067 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-config-loader | gaslighter-full | sonnet | 20260710-143019 | 1 | 1 | 1.00 | 3 | 0 | 7 | $0.1668 | none | none | b6a15246454262d2f973a975990092174fdb0a60 |
| hard-buried-constraints | baseline | haiku | 20260723-121556 | 0 | 1 | 1.00 | 2 | 0 | 11 | $0.0748 | __init__.py integration | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | baseline | haiku | 20260723-121556 | 1 | 1 | 0.90 | 2 | 1 | 9 | $0.0502 | __init__.py integration | unreachable return statement after loop | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | baseline | sonnet | 20260723-121556 | 0 | 1 | 0.90 | 2 | 0 | 8 | $0.3455 | __init__.py integration | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-lite | haiku | 20260723-121556 | 1 | 1 | 1.00 | 2 | 0 | 12 | $0.1102 | __init__.py integration | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-lite | haiku | 20260723-121556 | 0 | 1 | 1.00 | 2 | 0 | 10 | $0.1068 | __init__.py integration | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | baseline | sonnet | 20260723-121556 | 1 | 1 | 0.90 | 1 | 0 | 7 | $0.3831 | JSON payload encoding (sends plain text with JSON Content-Type header) | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | baseline | haiku | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 6 | $0.0345 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-lite | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 2 | 1 | 8 | $0.2626 | __init__.py integration | payload parameter (breaks handler pattern) | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-full | haiku | 20260723-121556 | 0 | 1 | 0.90 | 1 | 1 | 16 | $0.1364 | JSON payload encoding (sends plain text with JSON Content-Type header) | max_attempts variable | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | baseline | haiku | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 7 | $0.0357 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-lite | sonnet | 20260723-121556 | 0 | 1 | 0.90 | 1 | 1 | 9 | $0.4633 | JSON payload encoding and Content-Type header in _post helper | _post helper function extraction | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-lite | haiku | 20260723-121556 | 1 | 1 | 1.00 | 2 | 0 | 6 | $0.0444 | Type check for name field | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-lite | haiku | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 11 | $0.0693 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | baseline | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 8 | $0.1900 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-full | haiku | 20260723-121556 | 1 | 1 | 1.00 | 3 | 1 | 1 | $0.0836 | none | enhanced error messages | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-full | haiku | 20260723-121556 | 0 | 1 | 1.00 | 3 | 1 | 13 | $0.0735 | none | enhanced error messages | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-full | haiku | 20260723-121556 | 1 | 1 | 1.00 | 2 | 1 | 17 | $0.1697 | __init__.py integration | payload parameter (breaks handler pattern) | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | baseline | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 3 | 1 | 8 | $0.1973 | none | isinstance(price, bool) check | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-full | sonnet | 20260723-121556 | 0 | 1 | 0.90 | 2 | 0 | 9 | $0.4192 | __init__.py integration | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-full | sonnet | 20260723-121556 | 1 | 1 | 0.90 | 2 | 0 | 9 | $0.3230 | __init__.py integration | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-lite | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 8 | $0.1540 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-lite | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 8 | $0.2130 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | baseline | haiku | 20260723-121556 | 1 | 0 | 0.25 | 1 | 0 | 1 | $0.0774 | handler role parameter | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | baseline | haiku | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 11 | $0.1003 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-full | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 12 | $0.2172 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-full | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 11 | $0.2142 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-lite | haiku | 20260723-121556 | 0 | 1 | 0.75 | 2 | 1 | 11 | $0.0944 | migration 003 role column | migration 004_add_role_to_users | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-lite | haiku | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 11 | $0.1161 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-full | haiku | 20260723-121556 | 0 | 1 | 0.75 | 2 | 1 | 20 | $0.1089 | handler role default | DEFAULT_ROLE constant | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-full | haiku | 20260723-121556 | 1 | 1 | 0.75 | 2 | 0 | 21 | $0.1118 | validator role check | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-lite | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 14 | $0.2314 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-lite | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 14 | $0.1817 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | baseline | haiku | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 4 | $0.0277 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | baseline | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 14 | $0.2121 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | baseline | haiku | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 4 | $0.0281 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-lite | haiku | 20260723-121556 | 0 | 1 | 1.00 | 3 | 1 | 4 | $0.0368 | none | else: break statement in discount tier loop | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | baseline | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 4 | $0.1291 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | baseline | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 14 | $0.3780 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | baseline | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 4 | $0.1046 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-full | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 17 | $0.2310 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-full | haiku | 20260723-121556 | 0 | 1 | 1.00 | 3 | 1 | 15 | $0.0790 | none | dollar sign formatting in format_line_item output | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-lite | haiku | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 6 | $0.0677 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-full | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 17 | $0.2457 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-lite | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 4 | $0.1756 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | baseline | haiku | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 4 | $0.0351 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-full | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.1858 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-full | haiku | 20260723-121556 | 1 | 1 | 1.00 | 3 | 2 | 17 | $0.1070 | none | removal of str() conversions in Decimal instantiation | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | baseline | haiku | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 6 | $0.0486 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-lite | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.1928 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-full | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 8 | $0.2343 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-lite | haiku | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.0491 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | baseline | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.1310 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-lite | haiku | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.0592 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-full | haiku | 20260723-121556 | 1 | 1 | 1.00 | 3 | 1 | 5 | $0.0522 | none | self.steps as dict instead of list | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | baseline | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.1270 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-lite | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.1515 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | baseline | haiku | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.0358 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | baseline | haiku | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.0361 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-lite | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 6 | $0.2385 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-full | haiku | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 7 | $0.0777 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-full | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 8 | $0.1797 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-lite | haiku | 20260723-121556 | 0 | 1 | 1.00 | 3 | 1 | 5 | $0.0480 | none | _key_order | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-full | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 6 | $0.2169 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | baseline | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.1510 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-full | haiku | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 6 | $0.0464 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-full | haiku | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.0438 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-lite | haiku | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 7 | $0.0663 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | baseline | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.1506 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-lite | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.1700 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | baseline | haiku | 20260723-121556 | 0 | 1 | 0.91 | 3 | 0 | 5 | $0.0435 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-lite | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 6 | $0.2012 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-full | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 8 | $0.2436 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-full | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 8 | $0.2022 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | baseline | haiku | 20260723-121556 | 1 | 1 | 0.91 | 2 | 0 | 5 | $0.0506 | off() cannot remove handlers registered via once() | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-lite | haiku | 20260723-121556 | 1 | 1 | 0.91 | 2 | 0 | 8 | $0.0748 | off() cannot remove handlers registered via once() | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-full | haiku | 20260723-121556 | 0 | 1 | 0.91 | 3 | 0 | 5 | $0.0622 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-lite | haiku | 20260723-121556 | 0 | 1 | 0.91 | 2 | 0 | 6 | $0.0862 | off() cannot remove handlers registered via once() | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-full | haiku | 20260723-121556 | 1 | 1 | 0.91 | 2 | 0 | 6 | $0.0721 | off() cannot remove handlers registered via once() | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | baseline | sonnet | 20260723-121556 | 1 | 1 | 0.91 | 3 | 0 | 5 | $0.1533 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-lite | sonnet | 20260723-121556 | 1 | 1 | 0.91 | 3 | 0 | 5 | $0.1795 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | baseline | sonnet | 20260723-121556 | 0 | 1 | 0.91 | 3 | 1 | 5 | $0.1619 | none | _Entry helper class | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | baseline | haiku | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 4 | $0.0452 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-lite | sonnet | 20260723-121556 | 0 | 1 | 0.91 | 3 | 0 | 6 | $0.2113 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | baseline | haiku | 20260723-121556 | 0 | 1 | 0.88 | 2 | 1 | 5 | $0.0536 | dict type validation | ValueError exception catch is redundant | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-full | sonnet | 20260723-121556 | 0 | 1 | 0.91 | 3 | 0 | 6 | $0.1882 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | baseline | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 2 | 0 | 4 | $0.1339 | dict type validation | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | baseline | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.1580 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-full | haiku | 20260723-121556 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.0478 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-lite | haiku | 20260723-121556 | 1 | 1 | 0.75 | 2 | 0 | 4 | $0.0651 | should not cache error results | overly broad Exception catch instead of specific exceptions | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-lite | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 2 | 0 | 4 | $0.1570 | dict type validation | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-full | haiku | 20260723-121556 | 0 | 1 | 1.00 | 3 | 1 | 4 | $0.0666 | none | IOError exception catch is redundant | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-lite | haiku | 20260723-121556 | 0 | 1 | 0.88 | 2 | 0 | 4 | $0.0794 | should not cache error results | caching error results to _config_cache | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-lite | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 2 | 0 | 4 | $0.2006 | dict type validation | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-full | sonnet | 20260723-121556 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.2019 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-full | sonnet | 20260723-121556 | 1 | 1 | 0.91 | 3 | 0 | 8 | $0.2922 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-full | sonnet | 20260723-121556 | 1 | 1 | 1.00 | 1 | 0 | 7 | $0.2753 | dict type validation | regex pattern too restrictive with \w+ instead of [^}]+ | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | baseline | haiku | 20260723-150206 | 0 | 1 | 0.80 | 2 | 0 | 8 | $0.0406 | no explicit __init__.py export shown | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | baseline | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 1 | 7 | $0.0728 | none | JSON wrapping of body in dict | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | baseline | sonnet | 20260723-150206 | 1 | 1 | 0.90 | 2 | 1 | 8 | $0.2100 | Content-Type header | _post helper function | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | baseline | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 1 | 8 | $0.2359 | none | optional payload parameter with merging | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-lite | haiku | 20260723-150206 | 0 | 1 | 1.00 | 3 | 1 | 10 | $0.0794 | none | JSON wrapping of body in dict | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-full | haiku | 20260723-150206 | 0 | 1 | 1.00 | 3 | 2 | 13 | $0.0891 | none | optional payload parameter with timeout and specific exception handling | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-lite | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 2 | 9 | $0.0795 | none | helper functions _send_webhook_with_retry and _post_payload with flexible type handling | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | baseline | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 8 | $0.0460 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | baseline | haiku | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 6 | $0.0353 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-lite | haiku | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 10 | $0.0671 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-lite | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 8 | $0.0527 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-lite | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 1 | 8 | $0.2248 | none | optional payload parameter with merging | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-full | sonnet | 20260723-150206 | 0 | 1 | 0.90 | 2 | 0 | 9 | $0.2146 | Content-Type header | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-full | haiku | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 16 | $0.0818 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-full | sonnet | 20260723-150206 | 1 | 1 | 0.90 | 2 | 0 | 9 | $0.3479 | Content-Type header and last_error initialization | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-full | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 15 | $0.0842 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | baseline | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 8 | $0.2244 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | baseline | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 8 | $0.2244 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-lite | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 8 | $0.1794 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-lite | sonnet | 20260723-150206 | 1 | 1 | 0.90 | 2 | 0 | 9 | $0.3179 | Content-Type header | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-lite | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 8 | $0.1537 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | baseline | haiku | 20260723-150206 | 0 | 1 | 0.75 | 2 | 0 | 13 | $0.0817 | role value validation in validator module | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-full | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 11 | $0.1841 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | baseline | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 1 | 11 | $0.0912 | none | validation duplicated in User class and validator | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-buried-constraints | gaslighter-full | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 2 | 20 | $0.1848 | none | flexible payload vs template parameter handling with type detection | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-implicit-patterns | gaslighter-full | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 12 | $0.2241 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-lite | haiku | 20260723-150206 | 0 | 1 | 0.75 | 2 | 0 | 13 | $0.0820 | role value validation in validator module | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | baseline | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 14 | $0.1523 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-full | haiku | 20260723-150206 | 1 | 1 | 0.88 | 3 | 1 | 16 | $0.0846 | none | validation duplicated in User class and validator | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-lite | haiku | 20260723-150206 | 1 | 1 | 0.88 | 3 | 0 | 18 | $0.1391 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | baseline | haiku | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 4 | $0.0285 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | baseline | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 4 | $0.0282 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-lite | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 14 | $0.2046 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | baseline | sonnet | 20260723-150206 | 1 | 1 | 0.88 | 3 | 1 | 15 | $0.2689 | none | split migration (003 and 004) adds unnecessary complexity | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-full | haiku | 20260723-150206 | 0 | 1 | 0.75 | 2 | 0 | 21 | $0.1293 | role value validation in validator module | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-lite | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 4 | $0.0364 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-lite | haiku | 20260723-150206 | 0 | 1 | 1.00 | 3 | 1 | 5 | $0.0477 | none | rounding in format_line_item | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-lite | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 16 | $0.2973 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-full | haiku | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 8 | $0.0470 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | baseline | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 4 | $0.1341 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | baseline | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 4 | $0.1440 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-full | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 9 | $0.0590 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-full | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 23 | $0.4452 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-cascade-update | gaslighter-full | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 1 | 18 | $0.2499 | none | validation duplicated in User class and validator | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-lite | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 4 | $0.1620 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | baseline | haiku | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.0458 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | baseline | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.0461 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-full | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 8 | $0.1949 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-lite | haiku | 20260723-150206 | 0 | 1 | 1.00 | 3 | 1 | 5 | $0.0521 | none | method chaining (return self in add_step) not requested | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-lite | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 6 | $0.2182 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-preserve-behavior | gaslighter-full | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 7 | $0.1919 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-lite | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 1 | 4 | $0.0510 | none | dict for steps storage instead of list (semantically implies unordered, though order preserved in modern Python) | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | baseline | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.1321 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | baseline | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.1630 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-full | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 1 | 7 | $0.0662 | none | method chaining (return self in add_step) not requested | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | baseline | haiku | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.0361 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-full | haiku | 20260723-150206 | 0 | 1 | 1.00 | 3 | 2 | 9 | $0.0824 | none | comprehensive docstrings (Args/Returns/Raises) on every method for a simple data structure | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-full | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 6 | $0.2157 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-lite | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 6 | $0.2749 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-lite | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.1790 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | baseline | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.0371 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-lite | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.0457 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | baseline | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 5 | $0.1234 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-lite | haiku | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 6 | $0.0514 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-trailing-reqs | gaslighter-full | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 8 | $0.2183 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | baseline | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.1230 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-full | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.0447 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-full | haiku | 20260723-150206 | 0 | 1 | 1.00 | 3 | 1 | 6 | $0.0492 | none | self._order list for tracking insertion order | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-lite | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 6 | $0.1393 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | baseline | haiku | 20260723-150206 | 0 | 1 | 0.91 | 3 | 0 | 5 | $0.0476 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-lite | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 5 | $0.2041 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | baseline | haiku | 20260723-150206 | 1 | 1 | 0.91 | 3 | 0 | 6 | $0.0537 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-full | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 8 | $0.2109 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-registry-api | gaslighter-full | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 8 | $0.2068 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-lite | haiku | 20260723-150206 | 0 | 1 | 0.91 | 3 | 0 | 5 | $0.0675 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-full | haiku | 20260723-150206 | 0 | 1 | 0.91 | 3 | 0 | 6 | $0.0745 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-lite | haiku | 20260723-150206 | 1 | 1 | 0.91 | 3 | 0 | 5 | $0.0988 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | baseline | sonnet | 20260723-150206 | 1 | 1 | 0.91 | 3 | 0 | 5 | $0.1822 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | baseline | sonnet | 20260723-150206 | 0 | 1 | 0.91 | 3 | 2 | 5 | $0.2082 | none | _Entry class with __slots__ | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | baseline | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 3 | $0.0284 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | baseline | haiku | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 4 | $0.0344 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-full | haiku | 20260723-150206 | 1 | 1 | 0.91 | 3 | 1 | 6 | $0.0866 | none | wrapper function pattern for once() | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-lite | haiku | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 4 | $0.0479 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-lite | haiku | 20260723-150206 | 0 | 1 | 0.75 | 3 | 1 | 4 | $0.0490 | none | _expand_vars helper function | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-lite | sonnet | 20260723-150206 | 1 | 1 | 0.91 | 2 | 3 | 5 | $0.2214 | correct handling of multiple once() registrations for same handler | defaultdict, Counter imports and _once_counts tracking | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-full | haiku | 20260723-150206 | 1 | 1 | 0.88 | 3 | 0 | 5 | $0.0570 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-full | sonnet | 20260723-150206 | 0 | 1 | 0.91 | 3 | 1 | 8 | $0.2374 | none | list entries instead of tuples | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | baseline | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 1 | 4 | $0.1372 | none | isinstance(data, dict) check | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-lite | sonnet | 20260723-150206 | 0 | 1 | 0.91 | 3 | 1 | 6 | $0.2578 | none | _Entry class | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-full | haiku | 20260723-150206 | 0 | 1 | 0.75 | 3 | 1 | 6 | $0.0731 | none | _expand_vars helper function | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | baseline | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 1 | 4 | $0.1372 | none | isinstance(data, dict) check | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-event-bus | gaslighter-full | sonnet | 20260723-150206 | 1 | 1 | 0.91 | 3 | 2 | 8 | $0.2496 | none | _once_map tracking dictionary | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-lite | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 0 | 4 | $0.1656 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-lite | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 1 | 5 | $0.1947 | none | isinstance(data, dict) check | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-full | sonnet | 20260723-150206 | 0 | 1 | 1.00 | 3 | 1 | 5 | $0.1736 | none | isinstance(data, dict) check | 369080063d730a0d89e97e6e22e1fd970083907f |
| hard-config-loader | gaslighter-full | sonnet | 20260723-150206 | 1 | 1 | 1.00 | 3 | 0 | 7 | $0.2066 | none | none | 369080063d730a0d89e97e6e22e1fd970083907f |
