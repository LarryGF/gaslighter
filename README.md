...
![Works with Claude Code](https://img.shields.io/badge/works%20with-Claude%20Code-111111?style=flat-square) ![Works with omp](https://img.shields.io/badge/works%20with-omp-111111?style=flat-square) ![MIT license](https://img.shields.io/badge/license-MIT-111111?style=flat-square)
...
Merged across 4 eval runs (303 cells total: 8 tasks × 3 arms × 2 models, 1, 2, 2, 2 runs/cell across the runs):

| Arm | Correct | Auto Complete | Judge Completeness | Judge Overcorrection |
|---|---|---|---|---|
| baseline | 0.950 | 0.945 | 2.73 | 0.20 |
| gaslighter-lite | 0.990 | 0.961 | 2.69 | 0.26 |
| gaslighter-full | 0.990 | 0.952 | 2.71 | 0.39 |
...
gaslighter also runs under [omp](https://omp.sh) as a native extension from the
same decision engine. omp's `session_stop` event has a Claude/Codex-compatible
result shape (`decision:"block"`/`reason` or `continue`/`additionalContext`), so
`lite`/`full` map onto it almost exactly as they do for Claude — no follow-up-prompt
workaround needed. Install with `omp plugin install <this-repo>` (or `-l` for a
project-scoped install); the extension is declared in this repo's root
`package.json` (`omp.extensions`), no extra setup required. Same env vars as
Claude (`GASLIGHTER_MODE`, `GASLIGHTER_MAX_NUDGES`, etc.) apply. The extension
registers the shared `skills/*/SKILL.md` files through omp resource discovery, so
`gaslighter`, `config`, `eval`, and `judge` are available in both harnesses. Their
instructions are host-neutral: they use whichever skill, question, task, and shell
facilities the active harness provides.
...
node tests/test-agnostic.js         # shared core, env/store, OpenCode + omp adapters
