...
This lets gaslighter run under both Claude Code and OpenCode from one engine.
...
- `hooks/lib/` — shared agnostic core (engine/core/store/env/opencode)
...
- `tests/` — unit tests (`test-nudge-decision.js` Claude path, `test-agnostic.js` shared core + OpenCode)
